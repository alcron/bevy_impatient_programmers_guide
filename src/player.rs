...
fn spawn_player(
    ...
    let start_index = atlas_index_for(initial_facing, 0);
...
fn move_player(
    ...
    let (player_transform, player_animation_state) = &mut *player;
    ...
    if direction != Vec2::ZERO {
        ...
        player_animation_state.is_moving = true;

        if direction.x.abs() > direction.y.abs() {
            player_animation_state.facing = if direction.x > 0.0 {
                Facing::Right
            } else {
                Facing::Left
            };
        } else {
            player_animation_state.facing = if direction.y > 0.0 {
                Facing::Up
            } else {
                Facing::Down
            };
        ...
        player_animation_state.is_moving = false;
    }
}

fn animate_player(
    time: Res<Time>,
    mut query: Single<(&mut AnimationState, &mut AnimationTimer, &mut Sprite), With<Player>>,
) {
    let (anim, timer, sprite) = &mut *query;
    ...
    let just_started = anim.is_moving && !anim.was_moving;
    let just_stopped = !anim.is_moving && anim.was_moving;
    ...
    anim.was_moving = anim.is_moving;
}
